      ...
      <div className="mx-auto p-4 bg-gray-900 w-screen">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="mb-6 bg-white p-4 shadow rounded-lg">
            ...
            <div>
              <p className="mb-4">Monthly Credit:  {data?.user?.
                monthlyRequestCount}</p>
              <p className="mb-4">Credit Used: {data?.user?.
                apirequestCount}</p>
              <p className="mb-4">Credit Remaining: {data?.user?.
                monthlyRequestCount - data?.user?.
                  apirequestCount}</p>
              <p className="mb-4">Next Billing Date: {data?.user?.
                nextBillingDate ? new Date(data?.user?.
                  nextBillingDate).toDateString() : "Unpaid"}</p>
            </div>
          </div>
          ...
          <div className="mb-6 bg-white p-4 shadow rounded-lg">
            ...
            <div>
              ...
              {data?.user?.subscriptionPlan === 'Trial' && (
                <p className="mb-4 border rounded w-full py-2 px-3 text-gray-700 leading-tight">Trial:100 monthly request</p>
              )}
              {data?.user?.subscriptionPlan === 'Free' && (
                <p className="mb-4 border rounded w-full py-2 px-3 text-gray-700 leading-tight">
                  Free:100 monthly request</p>
              )}
              {data?.user?.subscriptionPlan === 'Basic' && (
                <p className="mb-4 border rounded w-full py-2 px-3 text-gray-700 leading-tight">
                  Free:150 monthly request</p>
              )}
              {data?.user?.subscriptionPlan === 'Premium' && (
                <p className="mb-4 border rounded w-full py-2 px-3 text-gray-700 leading-tight">
                  Free:300 monthly request</p>
              )}
              <Link
                to="/plans"
          ...
          <div className="mb-6 bg-white p-4 shadow rounded-lg">
            ...
            <div>
              <p className="mb-4">Trial Status:{data?.user?.trialActive ?
                < span className="text-green-500 font-bold"> Active</span> : <span className="text-yellow-500 font-bold"> Inactive</span>}</p>
                      ...
                      <li className="py-4 hover:bg-gray-50 transition duration-150 ease-in-out">
                        <div className="flex flex-col sm:flex-row justify-between">
                          ...
                          <div className="flex items-center">
                            <p className={`text-sm font-semibold ${payment.status === 'succeeded'
